...
@Component
@Slf4j
public class SmsCatChannelServiceImpl implements ISmsChannelService {
    ...
    private String  handleSendMsg(String content, String to, String username, String password, String deviceOtherInfo) {
        ...
        String query = String.format("http://sms-online.top:11186/api.jsp?act=sendSms&Port=%s&Receiver=%s&Sms=%s", deviceOtherInfo.split(":")[1],to, URLEncoder.encode(content,"UTF-8"));

        String result = HttpUtils.doGet(query);
        JSONObject jsonObject = JSON.parseObject(result);
        String code = jsonObject.getString("Code");
        if (code.equals("1")){
            String batchNo = jsonObject.getString("ID");
            return batchNo;
        }
        return null;
    }
