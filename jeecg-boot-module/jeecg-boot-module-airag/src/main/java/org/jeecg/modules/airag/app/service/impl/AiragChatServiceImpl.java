...
public class AiragChatServiceImpl implements IAiragChatService {
    ...
    @Autowired
    ConversationRecordsMapper conversationRecordsMapper;

    @Autowired
    ConversationMessageRecordsMapper conversationMessageRecordsMapper;
    ...
    public ChatConversation getOrCreateChatConversation(AiragApp app, String conversationId) {
        ...
        if (oConvertUtils.isNotEmpty(key)) {
            ...
            if (chatConversation!=null){
                List<ConversationMessageRecords> messageHistoryList = conversationMessageRecordsMapper.getByConversationId(chatConversation.getId());
                chatConversation.setMessages(messageHistoryList);
            }
        }
...
//        }
    ...
    private List<ChatMessage> collateMessage(ChatConversation conversation, String topicId) {
        List<ConversationMessageRecords> messagesHistory = conversation.getMessages();
    ...
    private void appendMessage(List<ChatMessage> messages, ChatMessage message, ChatConversation chatConversation, String topicId) {
        ...
        List<ConversationMessageRecords> histories = chatConversation.getMessages();
        if (oConvertUtils.isObjectEmpty(histories)) {
            histories = new ArrayList<>();
        }
        // 消息记录
        ConversationMessageRecords historyMessage = new ConversationMessageRecords();
        historyMessage.setConversationId(chatConversation.getId());
...
//        sendWithDefault(requestId, chatConversation, topicId, modelId, messages, aiChatParams);
     ...
     */
    private void sendWithDefault(String requestId, ChatConversation chatConversation, String topicId, String modelId, List<ChatMessage> messages, AIChatParams aiChatParams, ChatSendParams sendParams) {
        sendMsgByOne(requestId,topicId,chatConversation,messages,sendParams);
        // ai聊天响应逻辑
...
//            EventMessageData messageEventData = EventMessageData.builder().message("好的.").build();
...
//        }).start();
    ...
    void sendMsgByOne(String requestId, String topicId, ChatConversation chatConversation, List<ChatMessage> messages, ChatSendParams sendParams){
        EventData eventData = new EventData(requestId, null, EventData.EVENT_MESSAGE, chatConversation.getId(), topicId);
